...
import { executeResponseHandler } from "../helpers/response_handler";
    ...
    const handleSend = async (id: string) => {
      ...
      try {
        ...
        if (isCurrentRequest(reqID)) {
          ...
          try {
            // 检查是否有响应处理脚本
            const responseHandler = originalReq.responseHandler as string | undefined;
            if (responseHandler) {
              console.log('[Collection] Executing response handler');
              // 解码 body 以便脚本可以访问
              const resWithDecodedBody = { ...res };
              if (res.body) {
                try {
                  resWithDecodedBody.body = decode(res.body);
                } catch (e) {
                  // 如果解码失败，保持原样
                }
              }
              await executeResponseHandler(responseHandler, resWithDecodedBody);
            }
          } catch (handlerErr) {
            console.error('[Collection] Response handler failed:', handlerErr);
            showError(message, handlerErr);
          }
        }
      } catch (err) {
        if (isCurrentRequest(reqID)) {
