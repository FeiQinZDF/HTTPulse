...
import { reload, showError, isWebMode, downloadFileInBrowser, selectFileInBrowser } from "../helpers/util";
import { logoIcon } from "../icons";
import { exportTables, importTables } from "../commands/database";
import { exportAllData, importAllData } from "../services/database";
import { exportAPIConfigsToHTTP, exportAPIConfigsToJSON } from "../helpers/export";
...
export default defineComponent({
  ...
  setup() {
    ...
    const handleBackup = async () => {
      ...
      if (isWebMode()) {
        ...
        try {
          ...
          downloadFileInBrowser(filename, content);
          message.info(i18nSetting("exportTablesSuccess").replace("%s", filename));
        } catch (err) {
    ...
    const handleExportHTTP = async () => {
      try {
        const data = await exportAllData();
        const httpContent = exportAPIConfigsToHTTP(
          data.configs,
          environmentsStore.activeEnvironment || undefined
        );
        const filename = `${new Date().toISOString().split('T')[0]}.http`;
        downloadFileInBrowser(filename, httpContent);
        message.success(`已导出 ${data.configs.length} 个 API 到 ${filename}`);
      } catch (err) {
        showError(message, err);
      }
    };
    ...
    const handleExportJSON = async () => {
      try {
        const data = await exportAllData();
        const jsonContent = exportAPIConfigsToJSON(data.configs);
        const filename = `${new Date().toISOString().split('T')[0]}.json`;
        downloadFileInBrowser(filename, jsonContent);
        message.success(`已导出 ${data.configs.length} 个 API 到 ${filename}`);
      } catch (err) {
        showError(message, err);
      }
    ...
    const confirmRestoreWeb = async (file: File) => {
      try {
    ...
    const handleFunction = (key: string) => {
      switch (key) {
        ...
        case FnKey.importTables:
          handleRestore();
          break;
        case FnKey.exportHTTP:
          handleExportHTTP();
          break;
        case FnKey.exportJSON:
          handleExportJSON();
          break;
        default:
          break;
