...
import { exportAPIConfigsToHTTP } from "../../helpers/export";
import { getAPIFolderPath } from "../../helpers/folder_path";
  ...
  setup() {
    ...
    const handleExportHTTP = async () => {
      try {
        ...
        // 转换为 API 配置格式，包含完整的文件夹路径
        const folders = apiFolderStore.apiFolders;
        const configs = apiSettingStore.apiSettings.map((setting) => {
          const req = apiSettingStore.getHTTPRequest(setting.id);
          // 获取文件夹完整路径
          const folderPath = getAPIFolderPath(setting.id, folders);
          ...
          return {
            ...
            request: req,
            folder: folderPath,  // 使用完整路径，如 "用户管理/权限管理"
            tags: [],
            description: "",
            createdAt: Date.now(),
            updatedAt: Date.now(),
        ...
        downloadFileInBrowser(`${name}.http`, httpContent);
        message.success(`已导出 ${configs.length} 个 API 到 .http 文件`);
      } catch (err) {
        showError(message, err);
      }
    };
  ...
  render() {
    const options: DropdownMixedOption[] = [
      ...
      },
      {
        ...
        children: [
          {
            label: `${i18nCollection("exportSettings")} (.json)`,
            key: "export_json",
            icon: () => (
              <NIcon>
