...
interface FsParams {
  file: string;
  option: {
    dir: any;  // BaseDirectory from @tauri-apps/api/fs
  };
}
...
async function convertToFsParams(p: unknown) {
  const { BaseDirectory } = await import("@tauri-apps/api/fs");
  const option = {
      ...
      case Fn.rtf:
        {
          ...
          const params = await convertToFsParams(p);
