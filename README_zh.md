...
<h3 align="center">
<a href="https://github.com/vicanso/HTTPulse">HTTPulse</a>是基�?a href="https://github.com/tauri-apps/tauri">tauri</a>开发的跨平台API客户�?
</h3>
...
- 简单易用的操作及配置方�?
- 可快速导入postman，insomnia或者swagger的配�?
- 关键字筛选支持中文拼音或者首字母
...
仅调整前端界面时可直接使用浏览器的方式来测试(增加了各类mock的接�?，执行：
...
tiny配置了两个环境的ENV设置，其中`http://tiny.npmtrend.com`未生�?复选框未勾�?，如果需要切换不同的环境时，选择勾选不同的配置生效即可，需要注意不要同时选择相同的环境变量生效�?
...
在创建请求之后，则可以选择请求使用的env(自动添加至请求url�?，对应的HTTP Method以及输入URL。对于POST类请求的body部分，则可以选择对应的数据类型，如选择了json数据，填写对应的参数，图中的`{{md5(123123)}}`为函数形式，会在请求时执行此函数，填充对应的数据，后续会专门介绍此类函数�?
...
HTTPulse内置支持了部分函数，便于在请求中动态生成参数值，具体函数如下�?
...
读取文件并以字节的形式返回，其参数及使用方法与`readTextFile`一致，仅返回数据的类型不一�?
...
获取指定请求的响应，例如A请求的响应为`{"data": {"name": "HTTPulse"}}`，B请求需要使用A请求的响应的`data.name`，则先获取请求A的ID，在请求A的响应中点图标获取，如下�?
...
配置导入支持四种方式，`JSON`与`File`仅支持`HTTPulse`的配置形式导入，`PostMan`用于导入postman的配置，`Insonmia`则用于导入insonmia的配置�?
