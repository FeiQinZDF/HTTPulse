...
pub async fn export_tables() -> Result<String, HTTPulseError> {
    let download = download_dir().unwrap();

    let local = Local::now();

    let filename = format!("httpulse-backup-{}.zip", local.format("%Y-%m-%d"));
...
pub async fn import_tables(filename: String) -> Result<(), HTTPulseError> {
