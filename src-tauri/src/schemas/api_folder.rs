...
pub async fn delete_all_api_folder() -> Result<(), HTTPulseError> {
    let db = get_database().await;
...
pub async fn import_api_folder(data: Vec<serde_json::Value>) -> Result<(), HTTPulseError> {
    let db = get_database().await;
