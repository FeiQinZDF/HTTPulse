...
fn save_store(store: MutexGuard<CookieStore>) -> Result<(), HTTPulseError> {
    let filename = Path::new(get_app_dir()).join(COOKIE_FILE);
...
pub fn delete_cookie_from_store(c: Cookie) -> Result<(), HTTPulseError> {
    let name = c.name;
    if name.is_empty() {
        return Ok(());
    }
...
pub fn clear_cookie_from_store() -> Result<(), HTTPulseError> {
    let mut store = get_cookie_store();
    store.clear();
    save_store(store)?;
    Ok(())
...
pub fn save_cookie_store(set_cookies: Vec<String>, current_url: &Url) -> Result<(), HTTPulseError> {
    let mut store = get_cookie_store();
...
pub fn list_cookie() -> Result<Vec<String>, HTTPulseError> {
...
pub fn add_cookie(c: Cookie) -> Result<(), HTTPulseError> {
    let mut store = get_cookie_store();
